...
class TicketListPatimbanActivity : AppCompatActivity() {

    private lateinit var dbHelper: TicketDatabaseHelperPP
    private lateinit var recyclerView: RecyclerView
    private lateinit var adapter: TicketAdapterPP
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_ticket_list_patimban)

        dbHelper = TicketDatabaseHelperPP(this)

        recyclerView = findViewById(R.id.recyclerTicket)
        recyclerView.layoutManager = LinearLayoutManager(this)
        adapter = TicketAdapterPP(list)
        ...
        edtDari.setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_UP) showDatePicker(edtDari)
            true
        }
        edtSampai.setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_UP) showDatePicker(edtSampai)
            true
        }
    ...
    private fun loadTicketsRangeTanggalBuat(dariDdMmYyyy: String, sampaiDdMmYyyy: String) {
        ...
        val dariKey = ddMMyyyyToKey(dariDdMmYyyy)
        val sampaiKey = ddMMyyyyToKey(sampaiDdMmYyyy)
